...
class ModelMRI:
    def __init__(self, args: Namespace) -> None:
        ...
        self.net_mri = get_network(args.net, args.num_classes)
    ...
    def train(self):
        ...
        for epoch in range(self.args.ExpConfig.epochs):
            loss, loss_ce, loss_co = self.train_epoch(epoch)
            ...
            if epoch % self.args.val_freq == 0 or epoch == self.args.epochs - 1:
                loss, loss_ce, loss_co = self.validate_epoch(epoch)
                self.writer_train.add_scalar("LOSS", loss.data.item(), epoch)
                self.writer_train.add_scalar("LOSS_CE", loss_ce.item(), epoch)
                self.writer_train.add_scalar("LOSS_CO", loss_co.item(), epoch)
                if loss_best < loss:
...
def validate_epoch(self, epoch):
        ...
        with tqdm(total=num_val, desc=f'Epoch {epoch}', unit="image", leave=False) as pbar:
            for i, data in enumerate(self.loader_train):
                ...
                tgt = data["tgt"].to(dtype=torch.float32, device=self.device)
                ...
                with torch.no_grad(), torch.amp.autocast("cuda"):
                    ...
                    loss1 = self.cri_ce(out, tgt)
                    loss2 = self.cri_contrast(
                        feat_mri_img,
                        feat_img.to(self.device),
                        feat_mri_txt,
                        feat_txt.to(self.device),
                        data["contrastive"]
                    )
                    loss_epoch += (loss1 + loss2) / 2.
                    loss1_epoch += loss1
                    loss2_epoch += loss2
            pbar.update()
